        ...
        <div className="glass-panel config-container animate-slide-up">
          ...
          <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            <div>
              ...
              <input type="email" required placeholder="dev@email.com" value={email} onChange={e => setEmail(e.target.value)} />
            </div>
            <div>
              <label style={{ display: 'block', marginBottom: '0.5rem', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Пароль</label>
              <input type="password" required placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} />
            </div>
            {authError && <p style={{ color: 'var(--color-danger)', fontSize: '0.875rem' }}>⚠️ {authError}</p>}
